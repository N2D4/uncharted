...
export async function compileTypeScript(source: string): Promise<{ js: string }> {
    ...
    if (rootFile.statements.length !== 1 || rootFile.statements[0].kind !== ts.SyntaxKind.ExpressionStatement) {
    ...
    const transpiledJS = await new Promise<string>(resolve => program.emit(rootFile, (_, data) => resolve(data)));
